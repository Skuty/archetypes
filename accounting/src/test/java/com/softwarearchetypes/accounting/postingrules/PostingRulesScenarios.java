...
import com.softwarearchetypes.accounting.CreateAccount;
...
class PostingRulesScenarios {
    ...
    AccountingConfiguration accountingConfig = AccountingConfiguration.inMemory(clock);
    PostingRulesConfiguration postingRulesConfig = PostingRulesConfiguration.inMemory(accountingConfig.facade(), accountingConfig.eventPublisher(), clock);
    ...
    void should_execute_posting_rule_when_entry_is_created() {
        ...
        //and
        assertTrue(postingRulesFacade.saveRule(commissionRule).success());

        //when client payment of 1000 PLN is credited to receivables
        assertTrue(accountingFacade.transfer(incomingPaymentsAccount, receivablesAccount, pln(1000), TUESDAY_10_00, TUESDAY_10_00).success());
    ...
    void should_execute_posting_rules_in_priority_order_when_order_matters() {
        ...
        //and
        assertTrue(postingRulesFacade.saveRule(taxRule).success());
        assertTrue(postingRulesFacade.saveRule(bonusRule).success());

        //when 1000 PLN is credited to receivables
        assertTrue(accountingFacade.transfer(generateAssetAccount(), receivablesAccount, pln(1000), TUESDAY_10_00, TUESDAY_10_00).success());
    ...
    void should_save_and_find_posting_rule() {
        ...
        //when
        assertTrue(postingRulesFacade.saveRule(rule).success());
    ...
    void should_delete_posting_rule() {
        ...
        //when
        assertTrue(postingRulesFacade.deleteRule(rule.id()).success());
    ...
    void should_update_existing_posting_rule() {
        ...
        //and
        assertTrue(postingRulesFacade.saveRule(originalRule).success());
        ...
        assertTrue(postingRulesFacade.saveRule(updatedRule).success());
    ...
    void should_not_execute_posting_rule_when_eligibility_condition_is_not_met() {
        ...
        assertTrue(accountingFacade.transfer(generateAssetAccount(), otherAccount, pln(1000), TUESDAY_10_00, TUESDAY_10_00).success());
    ...
    private AccountId generateAssetAccount() {
        ...
        assertTrue(accountingFacade.createAccount(CreateAccount.generateAssetAccount(accountId, randomStringWithPrefixOf("acc"))).success());
        return accountId;
    }

    private AccountId generateOffBalanceAccount() {
        AccountId accountId = AccountId.generate();
        assertTrue(accountingFacade.createAccount(CreateAccount.generateOffBalanceAccount(accountId, randomStringWithPrefixOf("acc"))).success());
        return accountId;
    }
